...
def update_game_logic(left_lane_cars, right_lane_cars, top_lane_cars, bottom_lane_cars, left_lane_counter, top_lane_counter, right_lane_counter, bottom_lane_counter):
    ...
    if random.randint(1, CAR_DELAY) == 1 and len(left_lane_cars) < MAX_LEFT_LANE_CARS and left_lane_counter < MAX_LEFT_LANE_CARS:
        left_lane_cars.append([0, HEIGHT // 2 - CAR_RADIUS // 2])
        left_lane_counter += 1
    ...
    if random.randint(1, CAR_DELAY) == 1 and len(top_lane_cars) < MAX_TOP_LANE_CARS and top_lane_counter < MAX_TOP_LANE_CARS:
        top_lane_cars.append([WIDTH // 2 - CAR_RADIUS // 2, 0])
        top_lane_counter += 1
